...
class DBMS
{
    ...
    public void SelectAvg()
    {
        ...
        System.out.println("Average of Marks column is : "+(float)((float)iSum/(float)iCnt));               // To calculate Proper Output in point format we typecasted it
    }
    ...
    public void UpdateCity(int no, String str)
    {
        ...
        while(temp != null)
        {
            if(temp.Rno == no)                      // Rno is primary key
            {
                temp.City = str;
                break;                              // To stop the loop after the record gets updated
            }
            temp = temp.next;
        }
    ...
    public void DeleteFrom(int no)
    {
        ...
        while(temp.next != null)
        {
            if(temp.next.Rno == no)
            {
                temp.next = temp.next.next;                       // To skip the node which we want to delete and point to next node , GC will delete that skiped node automatically
                break;
            }
            temp = temp.next;
        }
...
class Program430
{
    public static void main(String Arg[])
    {
        ...
        obj.InsertIntoTable("Somesh","Vashi",42);
        obj.InsertIntoTable("Mansi","Nashik",53);
        obj.InsertIntoTable("Vedika","Nashik",74);
        obj.InsertIntoTable("Purva","Panvel",78);
        obj.InsertIntoTable("Roshan","Delhi",83);
